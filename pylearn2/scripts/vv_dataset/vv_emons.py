...
class VVEmons(DenseDesignMatrix):
    """
    A Pylearn2 Dataset class for accessing the data for the
    facial expression recognition Kaggle contest for the ICML
    2013 workshop on representation learning.
    ...
    def __init__(self, which_set,
            base_path = '${PYLEARN2_DATA_PATH}/vv_dataset',
            start = None,
            stop = None,
            preprocessor = None,
        ...
        files = {'train': 'emons_train.csv', 'public_test' : 'emons_test.csv'}
        ...
        super(VVEmons, self).__init__(X=X, y=y, view_converter=view_converter)
    ...
    def _load_data(self, path, expect_labels):
        ...
        for row in reader:
            if expect_labels:
                y_row_str, X_row_str = row
                #print "y_row_str: ", y_row_str
                y_row_strs = y_row_str.split(' ')
                y_row = map(lambda y: float(y), y_row_strs)
